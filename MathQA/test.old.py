...
def test_model(args, test_pairs, generate_num_ids, encoder, predict, generate, merge, output_lang, beam_size, save_test_path=None, debug=False):
    # 加载指定模型
    logger.info("testing -> " + os.path.join(args.model_reload_path))
    encoder.load_state_dict(torch.load(os.path.join(args.model_reload_path, "encoder.ckpt")))
    predict.load_state_dict(torch.load(os.path.join(args.model_reload_path, "predict.ckpt")))
    generate.load_state_dict(torch.load(os.path.join(args.model_reload_path, "generate.ckpt")))
    merge.load_state_dict(torch.load(os.path.join(args.model_reload_path, "merge.ckpt")))
    value_ac = 0
    equation_ac = 0
    eval_total = 0
...
def check_cl_model(args, test_pairs, generate_num_ids, encoder, predict, generate, merge, output_lang, beam_size):
    for test_pair in test_pairs:
        ...
        for test_batch in test_pair:
            test_res = evaluate_tree(test_batch[0], test_batch[1], generate_num_ids, encoder, predict, generate,
                                    merge, output_lang, test_batch[5], beam_size=beam_size)
            val_ac, equ_ac, _1, _2, _3, _4 = compute_prefix_tree_result(test_res, test_batch[2], output_lang, test_batch[4], test_batch[6])
            del test_res
            if val_ac:
                value_ac += 1
            if equ_ac:
...
if __name__=='__main__':
    ...
    # args.output_dir = "output/mwp-bert-en3"
    # args.model_reload_path = "output/mwp-bert-en3/epoch_91"
    args.output_dir = "output/mwp-bert-en-arithmetic"
    args.model_reload_path = "output/mwp-bert-en-arithmetic/epoch_100"

    # args.data_dir = "data"
    # args.train_file = "MathQA_bert_token_train.json"
    # args.finetune_from_trainset = "MathQA_bert_token_train.json"

    args.data_dir = "/data/qlh/Math-Plan/data/linear_expression/arithmetic/1"
    args.train_file = "arithmetic_train_mwp_format.json" # arithmetic_train_mwp_format | infer_train_mwp_format

    if debug:
        args.dev_file = "MathQA_bert_token_val.json"

        # args.test_file = "MathQA_bert_token_test.json"
        args.test_file = "MathQA_bert_token_val.json"
    else:
        args.test_file = "/data/qlh/Math-Plan/output/planning1212_g7/arithmetic/version_0/checkpoints/infer_valid_mwp_format.json"
        # args.test_file = "/data/qlh/Math-Plan/data/linear_expression/arithmetic/1/arithmetic_valid_mwp_format.json"
    ...
    if debug:
        ...
        val_data1 = json.load(open(args.dev_file, 'r', encoding='utf-8'))
    ...
    if debug:
        save_test_path = os.path.join(args.data_dir, args.test_file.replace("_format.json", ".json"))
    else:
        save_test_path = args.test_file.replace("_format.json", ".json")
    test_model(args, (test_pairs1, ), generate_num_ids,encoder, predict, generate, merge, output_lang, beam_size, save_test_path=save_test_path, debug=debug)
